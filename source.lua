    ...
    PageOne:Box("Kill", "string", function(Value)
        for _, TargetKill in pairs(Shorten(Value)) do
            if TargetKill.Character.Humanoid.Health > 0 and not table.find(getgenv().ViperWhitelist, TargetKill.UserId) then
                ...
                local Tween = TweenService:Create(Player.Character.HumanoidRootPart, TweenInfo.new(Speed, Enum.EasingStyle.Linear, EasingDirection.InOut), {CFrame = TargetKill.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 0)})
                Tween:Play()
