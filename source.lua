...
if PlaceId == 5278850819 then
    ...
    local PageTwo = WindowOne:CreateFolder("Miscellaneous")
    ...
    PageOne:Box("Kill", "string", function(Value)
        for _, TargetKill in pairs(Shorten(Value)) do
            if TargetKill.Character.Humanoid.Health > 0 and not table.find(getgenv().ViperWhitelist, TargetKill.UserId) then
                ...
                local Tween = TweenService:Create(Player.Character.HumanoidRootPart, TweenInfo.new(Speed, Enum.EasingStyle.Linear), {CFrame = TargetKill.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 0)})
                Tween:Play()
        ...
        for _, TargetGoto in pairs(Shorten(Value)) do
            if TargetGoto.Character.Humanoid.Health > 0 then
                pcall(function()
                    ...
                    local Tween = TweenService:Create(Player.Character.HumanoidRootPart, TweenInfo.new(Speed, Enum.EasingStyle.Linear), {CFrame = TargetGoto.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 0)})
                    Tween:Play()
                end)
            end
        end
    ...
    PageTwo:ColorPicker("ColorPicker", Color3.fromRGB(100, 0, 0), function(ColorValue)
        getgenv().ViperColor = ColorValue
    end)

    PageTwo:Button("Rejoin", function()
        TeleportService:Create(PlaceId, JobId, Player)
    end)

    PageTwo:Button("Server Hop", function()
        local ServerTables = {}
            ...
            for _, FindPlayer in pairs(Players:GetPlayers()) do
                if FindPlayer ~= Player and not table.find(getgenv().ViperWhitelist, FindPlayer.UserId) then
                    if FindPlayer.Character.Humanoid.Health > 0 then
                        if (FindPlayer.Character.HumanoidRootPart.Position - Spawn.Position).Magnitude > getgenv().SpawnDistance then
